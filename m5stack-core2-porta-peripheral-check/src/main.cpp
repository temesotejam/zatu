...
void drawPage(){M5.Display.fillScreen(0x0000);M5.Display.setTextSize(1);M5.Display.setTextColor(0xFFFF,0x0000);text(8,7,JP_TITLE,0xFFFF);if(mode==Mode::Servo){text(8,30,JP_SERVO,0x07FF);text(8,50,u8"PCA9685  /  I2C  G32=SDA  G33=SCL",0xBDF7);text(8,78,u8"\u4f4d\u7f6e\u3092\u30b9\u30e9\u30a4\u30c9\u3057\u3066\u8a2d\u5b9a",0xFFE0);text(8,172,u8"\u203b CH0\u3001 50Hz\u3001\u30bf\u30c3\u30c1\u4e2d\u306e\307f\u66f8\u8fbc",0xBDF7);}else if(mode==Mode::Tof){text(8,30,JP_TOF,0x07FF);text(8,50,u8"VL53L5CX  /  I2C  G32=SDA  G33=SCL",0xBDF7);text(8,78,u8"8x8 \u8ddd\u96e2\u30de\u30c3\u30d7 (mm)",0xFFE0);}else if(mode==Mode::Ina){text(8,30,JP_INA,0x07FF);text(8,50,u8"INA226  /  I2C  G32=SDA  G33=SCL",0xBDF7);text(8,78,u8"\u9ec4: \u96fb\u5727[V]  \u6c34: \u96fb\u6d41[A]",0xFFE0);}else{text(8,30,JP_MOTOR,0x07FF);text(8,50,u8"VESC  /  UART  G32=TX  G33=RX",0xBDF7);text(8,78,u8"Duty\u3092\u30b9\u30e9\u30a4\u30c9 (\u5b89\u5168\u4e0a +/-3%)",0xFFE0);text(8,172,u8"\u6307\u3092\u96e2\u3059\u3068 0% \u306b\u623b\u308a\u307e\u3059",0xBDF7);}tab(2,JP_SERVO,mode==Mode::Servo);tab(82,JP_TOF,mode==Mode::Tof);tab(162,JP_INA,mode==Mode::Ina);tab(242,JP_MOTOR,mode==Mode::Vesc);pageMode=mode;}
